...
def _load_hiv_dataset(input_path):
    """
    HIV — binary classification (1 = active, 0 = inactive).
    Follows MoleculeNet conventions.

    :param input_path:
    :return: smiles_list, rdkit_mol_objs_list, labels (with 0→-1)
    """
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['smiles']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]

    # HIV label column name (standard MoleculeNet)
...
class MoleculeDataset(InMemoryDataset):
    ...
    #from torch_geometric.data import Data  # optional

    def get(self, idx):
        out = self.data.__class__() if hasattr(self.data, '__class__') else Data()
        # AFTER (no warning)
    ...
    def process(self):
        ...
        if self.dataset == 'bace':
            ...
            for i in range(len(smiles_list)):
                ...
                data.y = torch.tensor([labels[i]])   # binary classification
