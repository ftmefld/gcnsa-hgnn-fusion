...
def onek_encoding_unk(value: int, choices: List[int]) -> List[int]:
    """
    Creates a one-hot encoding of length len(choices).
    If value is not in choices, falls back to the first slot (index 0).
    NOTE: We cannot add an extra 'unknown' bucket because other code
    assumes fixed atom feature dim (ATOM_FDIM == 89).
    """
    encoding = [0] * len(choices)
    try:
        index = choices.index(value)
    except ValueError:
        index = 0  # fallback without changing dimensionality
    encoding[index] = 1
    return encoding
...
class BatchMolGraph:
    def __init__(self, smiles, atom_fdim, bond_fdim, fp_fdim=None, data_name:str=""):
        ...
        # Only allocate FP container if we actually use fingerprints
        fp_x_out = None
        if self.fp_fdim is not None:
            fp_x_out = torch.empty((0, self.fp_fdim))

        data_file = open(
            f'.//dataset//{data_name}//raw//process_all.pkl',
            'rb'
        )
        ...
        for smi in smiles:
            ...
            # If using fingerprints, collect them (if present)
            if fp_x_out is not None and hasattr(mol_graph, "fp_x") and mol_graph.fp_x is not None:
                fp_x_out = torch.cat((fp_x_out, mol_graph.fp_x), dim=0)
        ...
        # Expose fp_x only if we built it
        self.fp_x = fp_x_out
