...
def inner_join_three_by_id(ids_a, Xa, ids_b, Xb, ids_c, Xc):
    ids_ab, Xa2, Xb2 = inner_join_by_id(ids_a, Xa, ids_b, Xb)
    pos_ab = {int(k): i for i, k in enumerate(ids_ab.tolist())}
    out_ids, ta, tb, tc = [], [], [], []
    pos_c = {int(k): i for i, k in enumerate(ids_c.tolist())}
    for k, i_ab in pos_ab.items():
        if k in pos_c:
            out_ids.append(k); ta.append(i_ab); tb.append(i_ab); tc.append(pos_c[k])
    if not out_ids:
        raise RuntimeError("No overlapping mol_id among the three sources.")
    out_ids = np.array(out_ids, dtype=np.int64)
    ta = np.array(ta, dtype=np.int64)
    tb = np.array(tb, dtype=np.int64)
    tc = np.array(tc, dtype=np.int64)
    return out_ids, Xa2[ta], Xb2[tb], Xc[tc]
...
class FusionDataset3(Dataset):
    def __init__(self, Xg: np.ndarray, Xa: np.ndarray, Xh: np.ndarray, Y: np.ndarray):
        X = np.concatenate([Xg, Xa, Xh], axis=1)
...
class GatedFusion3MLP(nn.Module):
    def __init__(self, dim_g: int, dim_a: int, dim_h: int, out_dim: int,
                 hidden: int = 768, dropout: float = 0.3):
        super().__init__()
        self.dim_g, self.dim_a, self.dim_h = dim_g, dim_a, dim_h
        h_g = hidden // 3 + (1 if hidden % 3 > 0 else 0)
        h_a = hidden // 3 + (1 if hidden % 3 > 1 else 0)
        h_h = hidden - (h_g + h_a)
        ...
        self.gate_h = nn.Sequential(nn.LayerNorm(dim_h), nn.Linear(dim_h, dim_h), nn.Sigmoid())

        self.proj_g = nn.Linear(dim_g, h_g)
        self.proj_a = nn.Linear(dim_a, h_a)
        self.proj_h = nn.Linear(dim_h, h_h)

        self.mlp = nn.Sequential(
            nn.LayerNorm(h_g + h_a + h_h),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Linear(h_g + h_a + h_h, h_g + h_a + h_h),
            nn.LayerNorm(h_g + h_a + h_h),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Linear(h_g + h_a + h_h, out_dim),
        )
        self._init_weights()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        xg = x[:, :self.dim_g]
        xa = x[:, self.dim_g:self.dim_g + self.dim_a]
        xh = x[:, self.dim_g + self.dim_a:]

        xg = xg * self.gate_g(xg)
        xa = xa * self.gate_a(xa)
        xh = xh * self.gate_h(xh)

        hg = self.proj_g(xg)
        ha = self.proj_a(xa)
        hh = self.proj_h(xh)

        h = torch.cat([hg, ha, hh], dim=1)
        return self.mlp(h)
...
def train_eval(args):
    ...
    id_h_tr, Xh_tr = load_npz_emb(args.hgn_train)
    id_h_va, Xh_va = load_npz_emb(args.hgn_val)
    id_h_te, Xh_te = load_npz_emb(args.hgn_test)

    # ---- join by id across all three (per split) ----
    ids_tr, Xg_tr, Xa_tr, Xh_tr = inner_join_three_by_id(id_g_tr, Xg_tr, id_a_tr, Xa_tr, id_h_tr, Xh_tr)
    ids_va, Xg_va, Xa_va, Xh_va = inner_join_three_by_id(id_g_va, Xg_va, id_a_va, Xa_va, id_h_va, Xh_va)
    ids_te, Xg_te, Xa_te, Xh_te = inner_join_three_by_id(id_g_te, Xg_te, id_a_te, Xa_te, id_h_te, Xh_te)
    ...
    if args.normalize:
        ...
        h_mu, h_sd = zscore_fit(Xh_tr); Xh_tr = zscore_apply(Xh_tr, h_mu, h_sd)

        Xg_va = zscore_apply(Xg_va, g_mu, g_sd)
        Xa_va = zscore_apply(Xa_va, a_mu, a_sd)
        Xh_va = zscore_apply(Xh_va, h_mu, h_sd)

        Xg_te = zscore_apply(Xg_te, g_mu, g_sd)
        Xa_te = zscore_apply(Xa_te, a_mu, a_sd)
        Xh_te = zscore_apply(Xh_te, h_mu, h_sd)

    # ---- datasets/loaders ----
    dtr = FusionDataset3(Xg_tr, Xa_tr, Xh_tr, y_tr)
    dva = FusionDataset3(Xg_va, Xa_va, Xh_va, y_va)
    dte = FusionDataset3(Xg_te, Xa_te, Xh_te, y_te)
    ...
    # ---- model ----
    dim_g = Xg_tr.shape[1]; dim_a = Xa_tr.shape[1]; dim_h = Xh_tr.shape[1]
    out_dim = dtr.Y.shape[1]
    model = GatedFusion3MLP(dim_g, dim_a, dim_h, out_dim, hidden=args.hidden, dropout=args.dropout).to(device)
    ...
    # return metrics for grid search
...
def main():
    ...
    ap.add_argument('--hgn_train', required=True)
    ap.add_argument('--hgn_val',   required=True)
    ap.add_argument('--hgn_test',  required=True)
    # labels
    ...
    ap.add_argument('--save_best', type=str, default='fusion3_best.pt')
    ap.add_argument('--save_logits', type=str, default='')

    # --- NEW: grid search options ---
    ap.add_argument('--do_gridsearch', action='store_true',
                    help='if set, run grid search over hyperparameters')
    ap.add_argument('--grid_hidden', type=str, default='',
                    help='comma-separated list of hidden dims, e.g. "512,768"')
    ap.add_argument('--grid_dropout', type=str, default='',
                    help='comma-separated list of dropouts, e.g. "0.2,0.3,0.4"')
    ap.add_argument('--grid_lr', type=str, default='',
                    help='comma-separated list of learning rates, e.g. "0.0005,0.001"')
    ap.add_argument('--grid_wd', type=str, default='',
                    help='comma-separated list of weight decays, e.g. "0.0,0.0001"')
    ...
    if not args.do_gridsearch:
        # normal single run (original behavior)
        train_eval(args)
        return
    ...
    for i, hp in enumerate(grid):
        ...
        print(f"Config {i+1}/{len(grid)}: "
              f"hidden={hp['hidden']}, dropout={hp['dropout']}, "
        ...
        # avoid overwriting single-run checkpoints/logits during grid search
        cfg.save_best = ""
        cfg.save_logits = ""

        best_auc, tr_auc, va_auc, te_auc = train_eval(cfg)
        print(f"[GRID] Config {i+1}: best_val_auc={best_auc:.6f}, "
              f"final_val_auc={va_auc:.6f}, test_auc={te_auc:.6f}")

        # choose by validation AUC (from best checkpoint)
        score = va_auc
        if score > best_val:
            best_val = score
            best_test = te_auc
